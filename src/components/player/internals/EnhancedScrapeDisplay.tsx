...
export function EnhancedScrapeDisplay({
  ...
  useEffect(() => {
    ...
    Object.values(sources).forEach(source => {
      if (source.status !== "waiting") {
        newLogs.push({
          timestamp: Date.now(),
          type: source.status === "success" ? "success" : source.status === "failure" ? "error" : "info",
          sourceId: source.id,
  ...
  const completedSources = Object.values(sources).filter(s =>
  ...
  const failedSources = Object.values(sources).filter(s => s.status === "failure" || s.status === "notfound").length;
      ...
      <div className="max-w-5xl w-full mx-auto">
        ...
        <div className="mb-6">
          <div className="flex items-center justify-between mb-2">
            <h2 className="text-2xl font-bold text-white">Finding Video Sources</h2>
            <button
          ...
          <div className="w-full bg-video-scraping-card bg-opacity-50 rounded-full h-2 mb-4">
            <div
              ...
              style={{ width: `${Math.round((completedSources / Math.max(totalSources, 1)) * 100)}%` }}
            />
          </div>
          ...
          <div className="grid grid-cols-3 gap-4 text-sm">
            <div className="text-center p-3 bg-video-scraping-card bg-opacity-30 rounded-lg">
              <div className="text-green-400 text-xl font-bold">{successfulSources}</div>
        ...
        <div className="grid lg:grid-cols-2 gap-6">
          ...
          <div className="space-y-4 max-h-96 overflow-y-auto">
            <h3 className="text-lg font-semibold text-white mb-3">Scraping Sources</h3>
            {sourceOrder.length === 0 ? (
              <div className="flex items-center justify-center h-32">
                <div className="text-center">
                  <StatusCircle type="loading" percentage={0} />
                  <p className="text-type-secondary mt-2">Initializing scrapers...</p>
                </div>
              </div>
            ) : (
              sourceOrder.map((order) => {
                ...
                return (
                  <div key={source.id} className="bg-video-scraping-card bg-opacity-50 rounded-lg p-4">
                    ...
                    {source.error && (
                      <div className="mt-3 p-3 bg-red-500/10 border border-red-500/30 rounded-md">
                        <p className="text-red-400 text-sm font-mono">{source.error}</p>
                      </div>
                    )}
                      ...
                      <div className="mt-3 space-y-2">
                        ...
                        <div className="space-y-2">
                          {order.children.map((embedId) => {
                            ...
                            return (
                              <div key={embedId} className="ml-8 bg-black/30 rounded p-2">
            ...
            <div className="space-y-4">
              ...
              <div className="bg-video-scraping-card bg-opacity-30 rounded-lg p-4">
                ...
                <div className="space-y-1 text-sm font-mono text-type-secondary">
                  ...
                  <div>Extension: {extensionActive ? "Active" : "Inactive"}</div>
              ...
              {logs.length > 0 && (
                <div className="bg-black/50 rounded-lg p-4 max-h-64 overflow-y-auto">
                  <h4 className="font-semibold text-white mb-2">Recent Events</h4>
                  <div className="space-y-1 text-xs font-mono">
                    {logs.map((log, i) => (
                      <div key={i} className="py-1 border-b border-gray-800/50">
                        ...
                        <span className={`ml-2 px-1 rounded ${getEventBadgeColor(log.type)}`}>
                          {log.type}
                        </span>
                        <span className="text-type-secondary ml-2">{log.message}</span>
                      </div>
                    ))}
                  </div>
                </div>
